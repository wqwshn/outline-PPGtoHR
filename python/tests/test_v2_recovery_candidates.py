...
def test_persistent_raw_bridge_can_acquire_a_penalty_overlapped_challenger() -> None:
    candidate = next(
        item
        for item in common_set_rescue_recovery_candidates_v1()
        if item.candidate_id == "persistent_raw_downward_bridge_v1"
    )
    machine = RecoveryStateMachine(candidate)
    observation = RecoveryObservation(
        window_kind="motion",
        current_track_bpm=95.0,
        current_track_delta_bpm=3.0,
        challenger_bpm=65.0,
        challenger_amp_ratio=0.8,
        challenger_stability_bpm=0.0,
        high_lock_risk_labels=(),
        challenger_near_penalty=True,
    )

    decisions = [machine.step(observation) for _ in range(2)]

    assert decisions[0].mode == "challenge"
    assert decisions[1].triggered is True
    assert decisions[1].output_bpm == pytest.approx(75.0)


def test_persistent_raw_bridge_keeps_true_rise_and_weak_challengers_bounded() -> None:
    candidate = next(
        item
        for item in common_set_rescue_recovery_candidates_v1()
        if item.candidate_id == "persistent_raw_downward_bridge_v1"
    ...
    near_track = RecoveryStateMachine(candidate).step(
        _observation(current_track_bpm=80.0, challenger_bpm=65.0)
    )
    weak = RecoveryStateMachine(candidate).step(
        RecoveryObservation(
            window_kind="motion",
            current_track_bpm=95.0,
            current_track_delta_bpm=0.0,
            challenger_bpm=65.0,
            challenger_amp_ratio=0.4,
            challenger_stability_bpm=0.0,
            high_lock_risk_labels=(),
            challenger_near_penalty=True,
        )
    ...
    assert near_track.suppressed_reason == "relative_gap_too_small"
    assert weak.suppressed_reason == "weak_challenger"


def test_identity_blind_dual_rescue_routes_woli_and_xiezi_without_scene_labels() -> None:
    candidate = next(
        item
        for item in common_set_rescue_recovery_candidates_v1()
        if item.candidate_id == "identity_blind_dual_high_lock_rescue_v1"
    )

    woli = RecoveryStateMachine(candidate)
    woli_observations = [
        RecoveryObservation(
            window_kind="motion",
            current_track_bpm=current,
            current_track_delta_bpm=0.0,
            challenger_bpm=value,
            challenger_amp_ratio=0.8,
            challenger_stability_bpm=0.0,
            high_lock_risk_labels=("protected_wrong_track",),
            challenger_near_penalty=near_penalty,
            challenger_owned_penalty_support=near_penalty,
            unpenalized_previous_support_visible=True,
        )
        for current, value, near_penalty in (
            (113.89, 77.64, False),
            (112.39, 80.93, False),
            (110.89, 86.43, False),
        )
    ]
    woli_decisions = [woli.step(observation) for observation in woli_observations]

    xiezi = RecoveryStateMachine(candidate)
    xiezi_observations = (
        RecoveryObservation(
            window_kind="motion",
            current_track_bpm=87.93,
            current_track_delta_bpm=0.0,
            challenger_bpm=65.55,
            challenger_amp_ratio=0.58,
            challenger_stability_bpm=0.0,
            high_lock_risk_labels=(
                "late_rank",
        ...
        RecoveryObservation(
            ...
            current_track_bpm=87.93,
            current_track_delta_bpm=0.0,
            challenger_bpm=66.28,
            challenger_amp_ratio=0.8,
            challenger_stability_bpm=0.73,
            high_lock_risk_labels=(
                "held_previous",
                "protected_wrong_track",
                "near_motion_peak",
    ...
    xiezi_decisions = [xiezi.step(item) for item in xiezi_observations]

    assert woli_decisions[1].triggered is False
    assert woli_decisions[2].triggered is True
    assert woli_decisions[2].trace["rescue_branch"] == "mid_band_rise_guard"
    assert xiezi_decisions[0].triggered is False
    assert xiezi_decisions[1].triggered is True
    assert xiezi_decisions[1].trace["rescue_branch"] == "lower_band_peak_bridge"


def test_identity_blind_dual_rescue_preserves_the_existing_base_route() -> None:
    candidate = next(
        item
        for item in common_set_rescue_recovery_candidates_v1()
        if item.candidate_id == "identity_blind_dual_high_lock_rescue_v1"
    ...
    observation = RecoveryObservation(
        window_kind="motion",
        current_track_bpm=126.0,
        current_track_delta_bpm=3.5,
        challenger_bpm=96.7,
        challenger_amp_ratio=0.8,
        challenger_stability_bpm=0.0,
        high_lock_risk_labels=("held_previous",),
    ...
    assert decision.mode == "challenge"
    assert decision.suppressed_reason == ""
    assert decision.trace["rescue_branch"] == "base"

    high_rate_base = RecoveryStateMachine(candidate).step(
        RecoveryObservation(
            window_kind="motion",
            current_track_bpm=180.0,
            current_track_delta_bpm=3.5,
            challenger_bpm=155.0,
            challenger_amp_ratio=0.8,
            challenger_stability_bpm=0.0,
            high_lock_risk_labels=("late_rank",),
            challenger_near_penalty=False,
            unpenalized_previous_support_visible=False,
        )
    )
    assert high_rate_base.mode == "challenge"
    assert high_rate_base.effective_gap_bpm == pytest.approx(20.0)


def test_identity_blind_dual_rescue_rejects_captured_cross_scene_lookalikes() -> None:
    candidate = next(
        item
        for item in common_set_rescue_recovery_candidates_v1()
        if item.candidate_id == "identity_blind_dual_high_lock_rescue_v1"
    ...
    lookalikes = (
        ...
        RecoveryObservation(
            window_kind="motion",
            current_track_bpm=78.3,
            current_track_delta_bpm=0.0,
            challenger_bpm=57.1,
            challenger_amp_ratio=0.8,
            challenger_stability_bpm=0.0,
            high_lock_risk_labels=("held_previous", "protected_wrong_track"),
            challenger_near_penalty=True,
            unpenalized_previous_support_visible=False,
        ),
        RecoveryObservation(
            window_kind="motion",
            current_track_bpm=118.65,
            current_track_delta_bpm=-1.29,
            challenger_bpm=83.5,
            challenger_amp_ratio=0.8,
            challenger_stability_bpm=0.0,
            high_lock_risk_labels=("late_rank",),
    ...
    decisions = [RecoveryStateMachine(candidate).step(item) for item in lookalikes]

    assert all(decision.triggered is False for decision in decisions)
    assert all(decision.mode == "locked" for decision in decisions)
    assert [decision.suppressed_reason for decision in decisions] == [
        "identity_blind_gap_above_branch_max",
        "candidate_below_fixed_floor",
        "identity_blind_rise_guard_risk_incomplete",
    ]
