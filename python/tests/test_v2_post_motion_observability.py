...
def test_delayed_raw_bootstrap_retries_after_two_dominant_consistent_windows() -> None:
    rows = (
        {
            "center_s": 132.0,
            "archived_final_bpm": 135.8,
            "handoff_bpm": 96.7,
            "handoff_trace": {
                "source": "raw_initial_fallback",
                "predicted_prior_bpm": 136.3,
                "selected_rank": 1,
            },
            "qualification_reason": "insufficient_history",
            "switch_target_ready": False,
            "switch_target_readiness_reason": "candidate_not_qualified",
            "raw_top5": ((96.7, 12.0), (194.1, 3.0)),
        },
        {
            "center_s": 133.0,
            "archived_final_bpm": 133.3,
            "handoff_bpm": 95.9,
            "handoff_trace": {
                "source": "raw_local_peaks",
                "predicted_prior_bpm": 136.8,
                "selected_rank": 1,
            },
            "qualification_reason": "insufficient_history",
            "switch_target_ready": False,
            "switch_target_readiness_reason": "candidate_not_qualified",
            "raw_top5": ((95.9, 13.5), (191.9, 4.0)),
        },
        {
            "center_s": 134.0,
            "archived_final_bpm": 133.3,
            ...
            "handoff_trace": {
                "source": "raw_local_peaks",
                "predicted_prior_bpm": 137.3,
                "selected_rank": 1,
            },
            "qualification_reason": "insufficient_history",
            "switch_target_ready": False,
            "switch_target_readiness_reason": "candidate_not_qualified",
            "raw_top5": ((96.7, 15.0), (191.9, 4.4)),
        },
    )

    frozen = causal_bootstrap_timeline(rows, motion_end_s=131.0)
    rescued = causal_bootstrap_timeline(
        rows,
        motion_end_s=131.0,
        config=FrozenDualResetConfig(delayed_raw_bootstrap_hits=2),
    )

    assert frozen["final_bpm"] == (135.8, 133.3, 133.3)
    assert frozen["bootstrap_reason"] == "source_not_raw_local_peaks"
    assert rescued["final_bpm"] == (135.8, 95.9, 96.7)
    assert rescued["switch_states"] == (
        "archived_final",
        "bootstrap_provisional",
        "bootstrap_provisional",
    )
    assert rescued["bootstrap_reason"] == (
        "delayed_raw_dominant_after:source_not_raw_local_peaks"
    )


def test_delayed_raw_bootstrap_rejects_non_dominant_raw_targets() -> None:
    rows = tuple(
        {
            "center_s": 132.0 + index,
            "archived_final_bpm": 135.0,
            "handoff_bpm": 96.0 + index,
            "handoff_trace": {
                "source": "raw_initial_fallback" if index == 0 else "raw_local_peaks",
                "predicted_prior_bpm": 136.0,
                "selected_rank": 1,
            },
            "qualification_reason": "insufficient_history",
            "switch_target_ready": False,
            "switch_target_readiness_reason": "candidate_not_qualified",
            "raw_top5": ((96.0 + index, 10.0), (190.0, 4.0)),
        }
        for index in range(3)
    )

    result = causal_bootstrap_timeline(
        rows,
        motion_end_s=131.0,
        config=FrozenDualResetConfig(delayed_raw_bootstrap_hits=2),
    )

    assert result["final_bpm"] == (135.0, 135.0, 135.0)
