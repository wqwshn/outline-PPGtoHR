...
class V2RunConfig:
    ...
    # Opt-in rescue: after an already-active handoff loses current PPG
    # observability for this many consecutive windows, transfer the remaining
    # suffix to the archived Final path.  ``None`` preserves the irreversible
    # production handoff exactly.
    post_motion_minimal_loss_fallback_hits: int | None = None
    # Opt-in rescue: after the initial provisional bootstrap is rejected,
    # admit a delayed retry once this many consecutive post-motion windows
    # expose the same dominant rank-1 raw target. ``None`` preserves PM-CHR.
    post_motion_delayed_raw_bootstrap_hits: int | None = None
    ...
    low_reacquire_candidate_id: str | None = None
