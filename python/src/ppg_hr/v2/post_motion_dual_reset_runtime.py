...
POST_MOTION_CAUSAL_HANDOFF_LOSS_FALLBACK = (
    "post_motion_causal_handoff_loss_fallback_v1"
)
POST_MOTION_CAUSAL_HANDOFF_DELAYED_BOOTSTRAP = (
    "post_motion_causal_handoff_delayed_bootstrap_v1"
)
...
class FrozenDualResetConfig:
    ...
    minimal_loss_fallback_hits: int | None = None
    delayed_raw_bootstrap_hits: int | None = None
    delayed_raw_bootstrap_dominance_ratio: float = 3.0
    delayed_raw_bootstrap_continuity_bpm: float = 3.0
    delayed_raw_bootstrap_target_match_bpm: float = 1.0
        ...
        minimal = run_minimal_handoff(
            tuple(
                MinimalHandoffInput(
                    ...
                    current_observability_lost=(
                        row["observability_state"] == "lost_after_recovery"
                    ),
            ...
            config=MinimalHandoffConfig(
                ...
                loss_fallback_hits=cfg.minimal_loss_fallback_hits,
            ),
        )
        timeline = {
...
def _raw_top_peak(row: dict[str, Any]) -> tuple[float, float, float | None] | None:
    raw_top5 = row.get("raw_top5")
    if isinstance(raw_top5, str):
        raw_top5 = json.loads(raw_top5)
    if not raw_top5 or len(raw_top5[0]) < 2:
        return None
    bpm = float(raw_top5[0][0])
    amplitude = float(raw_top5[0][1])
    runner_up = (
        float(raw_top5[1][1])
        if len(raw_top5) > 1 and len(raw_top5[1]) >= 2
        else None
    )
    if not math.isfinite(bpm) or not math.isfinite(amplitude):
        return None
    return bpm, amplitude, runner_up


def _delayed_raw_bootstrap_start(
    rows: Sequence[dict[str, Any]],
    *,
    motion_end_s: float,
    config: FrozenDualResetConfig,
) -> int | None:
    required = config.delayed_raw_bootstrap_hits
    if required is None or int(required) < 1:
        return None
    ...
    for index, row in enumerate(rows):
        ...
        if elapsed <= 0.0:
            continue
        if elapsed > config.bootstrap_confirmation_deadline_s:
            break
        trace = _mapping(row.get("handoff_trace", {}))
        peak = _raw_top_peak(row)
        target = float(row["handoff_bpm"])
        archived = float(row["archived_final_bpm"])
        dominance = (
            float("inf")
            if peak is not None and (peak[2] is None or peak[2] <= 0.0)
            else (peak[1] / peak[2] if peak is not None else 0.0)
        )
        eligible = bool(
            trace.get("source") in {"raw_initial_fallback", "raw_local_peaks"}
            and int(trace.get("selected_rank") or 0) == 1
            and peak is not None
            and abs(target - peak[0])
            <= config.delayed_raw_bootstrap_target_match_bpm
            and dominance >= config.delayed_raw_bootstrap_dominance_ratio
            and abs(target - archived) >= config.gap_rescue_gap_bpm
            and row.get("qualification_reason") != "unreliable"
        )
        continuous = bool(
            eligible
            and (
                previous_target is None
                or abs(target - previous_target)
                <= config.delayed_raw_bootstrap_continuity_bpm
            )
        )
        hits = hits + 1 if continuous else (1 if eligible else 0)
        previous_target = target if eligible else None
        if hits >= int(required):
            return index
    return None
...
def causal_bootstrap_timeline(
    ...
    *,
    ...
    if reason != "admitted":
        ...
        delayed_start = _delayed_raw_bootstrap_start(
            rows,
            motion_end_s=motion_end_s,
            config=cfg,
        )
        ...
        start_index = delayed_start
        bootstrap_reason = f"delayed_raw_dominant_after:{reason}"
        for index in range(1, start_index):
            switch_reasons[index] = "delayed_raw_bootstrap_observing"
    ...
    for index, row in enumerate(rows):
        ...
        archived = float(row["archived_final_bpm"])
        peak = _raw_top_peak(row)
        raw_top1 = peak[0] if peak is not None else None
        guarded = bool(
            not confirmed
