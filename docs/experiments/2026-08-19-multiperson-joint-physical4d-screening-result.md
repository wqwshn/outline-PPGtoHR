...
- 最终时延合同：`full_mae_evaluation_time_bias_v1`
...
最终共同窗口 MAE 均值为 1.4875 BPM，中位数为 1.4933 BPM，范围为 0.5863–3.0172 BPM。相对固定 5 s，40/48 条改善、8/48 条不变、0/48 条退化；平均改善 0.1035 BPM，中位改善 0.0426 BPM，最大改善 0.3529 BPM。由于 5 s 本身属于候选且所有候选共用同一评价窗口，因此“零退化”是机制合同的确定性性质，不是事后筛除结果。

新偏置使一条记录 `run2_LZJ_0711` 的经典门槛诊断由通过变为未通过，原因是 `right_censored_e10`。门槛复评只作诊断，不改变偏置、4D 坐标或面板组成；最终仍是原冻结的 48 条记录。
...
五个偏置先取共同拥有有限参考值、有限预测值且窗口可靠的交集，然后在完全相同的窗口上计算全段 MAE并直接选择最小值。完全并列时先选距 5 s 最近者，再选较小偏置。不设置最小改善或领先次优阈值，也不再使用 R-pre、R-post 或 R-all；R-all 仅属于开发过程，旧派生结果已由本合同原位覆盖，追溯入口为 Git 历史与覆盖前冻结快照。

正式数据集 MAE 使用上述共同窗口合同。按每个偏置自身最大参考重叠范围计算的完整 MAE、运动 MAE、E10/E20、L10/L20、恢复期、右删失和真实上升指标保留为兼容性及门槛诊断，不与正式共同窗口 MAE 混算。

## 时延与 MAE 汇总

- 最终偏置计数：4.0 s 为 16 条，4.5 s 为 12 条，5.0 s 为 8 条，5.5 s 为 3 条，6.0 s 为 9 条。
- 共同可靠窗口数：均值 196.67，中位 178，范围 170–265。
- 正式共同窗口 MAE：均值 1.4875 BPM，中位 1.4933 BPM，范围 0.5863–3.0172 BPM。
- 相对固定 5 s 的正式改善：均值 0.1035 BPM，中位 0.0426 BPM，范围 0–0.3529 BPM；40 条改善、8 条不变、0 条退化。
- 最大重叠兼容 MAE：均值 1.4874 BPM，中位 1.4933 BPM，范围 0.5863–3.0172 BPM。该值只用于兼容诊断。
- 相比旧 R-all 偏置，27/48 条最终偏置发生变化；这是在同一冻结4D轨迹上按最终全段目标重新裁决，不是重新选择算法参数。
- 经典门槛复评：47/48 条通过，1/48 条未通过；相对筛选时有 1 条由通过变为未通过，未触发重筛。
...
| run | `run2_LZJ_0711` | 5.0 | 2.9014 | 2.9014 | 0.0000 | 未通过：`right_censored_e10` |
...
1. 收尾验证绑定了评价器提交、48 个已选 solver report 哈希、48 条数据/参考哈希、冻结坐标、数据集卡片、结果汇总以及覆盖前输入快照。
...
- `dataset_card.json`：48 条冻结记录、数据/参考哈希、最终偏置、正式共同窗口 MAE 与已选坐标；
- `bias_manifest.json`：48 条五点曲线、共同窗口哈希、兼容性指标、门槛诊断及输入 report 哈希；
