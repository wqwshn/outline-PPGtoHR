...
- 最终时延合同：`gate_aware_full_mae_time_bias_v3`
...
原始全段 MAE 最小者的共同窗口 MAE 均值为 1.4875 BPM。门槛优先选择后，正式有效 MAE 均值为 1.4876 BPM，中位数为 1.4933 BPM，范围为 0.5863–3.0172 BPM；相对固定 5 s，40/48 条改善、7/48 条不变、1/48 条因门槛约束轻微增加0.0018 BPM，平均改善仍为0.1035 BPM。

`run2_LZJ_0711` 的原始全段 MAE 最小偏置5.0 s触发 `right_censored_e10`。五偏置复评中4.0和4.5 s均通过，最终选择其中 MAE 更低的4.5 s：正式 MAE 2.9032 BPM，仅比5.0 s高0.0018 BPM。该选择不读取上一轮 R-all 偏置。最终48/48条均通过经典门槛，五项全失败风险数为0；4D坐标和面板组成不变。
...
五个偏置先取共同拥有有限参考值、有限预测值且窗口可靠的交集，在完全相同的窗口上计算全段 MAE；同时对每个偏置的候选和基线使用相同偏置复评 `multiperson_joint_screening_gate_v1`。只要存在门槛通过项，就在通过项中选择全段 MAE 最低者；五项全失败时仍选择全局最低 MAE，并记录 `no_gate_passing_time_bias_candidate` 风险。完全并列时先选距5 s最近者，再选较小偏置。R-pre、R-post、R-all及任何历史偏置均不参与最终选择。
...
- 门槛优先后的最终有效偏置计数：4.0 s 为 16 条，4.5 s 为 13 条，5.0 s 为 7 条，5.5 s 为 3 条，6.0 s 为 9 条。
...
- 正式有效共同窗口 MAE：均值 1.4876 BPM，中位 1.4933 BPM，范围 0.5863–3.0172 BPM。
- 相对固定 5 s 的正式有效改善：均值 0.1035 BPM，中位 0.0426 BPM，范围 -0.0018–0.3529 BPM；40 条改善、7 条不变、1 条因门槛约束轻微增加误差。
- 最大重叠兼容 MAE：均值 1.4874 BPM，中位 1.4933 BPM，范围 0.5863–3.0172 BPM。该值只用于兼容诊断。
- 相比旧 R-all 历史偏置，27/48 条正式有效偏置发生变化；历史偏置不参与选择。
- 门槛约束改变了1条记录的全局最低 MAE选择，即 `run2_LZJ_0711`；最终48/48条通过，五项全失败风险为0。
...
| run | `run2_LZJ_0711` | 4.5 | 2.9032 | 2.9014 | -0.0018 | 通过（5.0 s因`right_censored_e10`未入选） |
...
1. 收尾验证绑定了评价器提交、48 个已选 solver report 哈希、48 条数据/参考哈希、冻结坐标、数据集卡片、结果汇总以及覆盖前输入快照；每条记录的五偏置 MAE、五偏置门槛、最终选择与风险标记均可重算。
...
- `bias_manifest.json`：48 条五点曲线、共同窗口哈希、五偏置门槛、最终选择、风险标记及输入 report 哈希；
