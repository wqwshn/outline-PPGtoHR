...
**核心思路**：运动结束后继续跑自适应滤波和 FFT 两条链，从运动结束点向后扫描 FFT 曲线首次 >= 自适应曲线的时间点（交叉点）。运动结束到交叉点为"恢复段"，归类为静息。自适应滤波最多延续 `max_recovery_seconds=30.0` 秒，超过则强制切 FFT。
...
1. 对 FFT 列（col 2）和自适应列（col 3/raw）分别执行 `smoothdata_movmedian(win_len=smooth_win_len)`。
...
`V2RunConfig.max_recovery_seconds: float = 30.0`
