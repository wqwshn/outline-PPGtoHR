...
### 16.1 2026-07-28 滤波稳定性审计预算修订

第 16 节的 `672/684/1368` 是机制实验主体的原始冻结上限。组合库进入主体实验前需要先取得稳定性与频谱回执，因此另立 `filter_profile_stability_audit` 诊断阶段；该阶段不产生完整心率追踪轨迹，不运行探索或独立 BO，也不改变后续候选建议历史。

首次审计冻结 8 个档位 × 4 条代表记录，共 32 个唯一身份。结果显示 6 个档位通过全部非尾首比稳定性门，而两个边界档位存在真实失稳；同时，旧实现把冷启动近零分母导致的单点 `tail/head max` 作为硬门，会把其余稳定档位机械误杀。修正契约保留该最大值及 P95 作为描述量，但硬门继续由稳定性负载、权重范数、残差 RMS P95、非有限值和频谱指标承担。

人工批准的唯一修订为：

| 项目 | 数量 | 处理 |
|---|---:|---|
| 未改变的 6 个档位 × 4 条记录 | 24 | 复用既有数值结果，只按修正契约重分类，不新增求解 |
| 两个替换边界档位 × 4 条记录 | 8 | 新增 8 个唯一诊断身份 |
| 独立 BO / 探索 / 完整心率轨迹 | 0 | 继续禁用 |

修订后的滤波稳定性审计总量为 40 个唯一身份，机制实验主体正常上限由 672 增至 712，含三折身份补算的绝对上限由 684 增至 724，最坏操作尝试上限由 1368 增至 1448。预算申请哈希为 `40dcfd55d1d11a298eb0cc489de0a3b1d3af3e485e15c24951c9bbd6e515d32f`，修订组合库哈希为 `c049d8f88775fb5cb212f70cfa90cac716b60cf0afcdb72b7781453b3ad22a28`。

执行结果必须同时满足：只新增上述 8 个身份、只重分类上述 24 个缓存结果、探索与独立 BO 计数均为 0。实际完成回执记录 40 个计划/40 个实际唯一运行、40 个缓存证据、1 次既有失败尝试和 1 次同身份重试；新增替换运行数为 8、重分类复用数为 24，完整心率轨迹与独立 BO均为 0。

代码审阅随后发现实现曾把本 spec 第 7.5 节冻结的 `median(pulse_power_retention) >= 0.80` 错写为 `0.25`。该实现值及其“8/8 通过”结论无效，不得作为后续矩阵入口。恢复 `0.80` 硬门后，只对既有 32 个最终档位—记录摘要重分类，新增数值求解为 0；正确契约哈希为 `8e46582997dc08db81a7430d4443c0b84bdbc2558c4f638afce1fbd67a667ac4`。最终只有 `p25-short-low`、`p25-short-mid`、`p25-long-mid` 和 `p50-short-low` 通过，另外 4 个档位被拒绝，中间与激进恢复哨兵均失去资格。因此本阶段状态为 `blocked_insufficient_eligible_profiles`，在另一次人工预算修订前不得进入后续矩阵。
...
- [ ] 机制实验主体是否不超过原始 `684/1368` 上限，且连同已批准的 40 个滤波审计身份后不超过修订上限 `724/1448`？
- [ ] 完整独立 BO 是否默认处于禁用状态？
