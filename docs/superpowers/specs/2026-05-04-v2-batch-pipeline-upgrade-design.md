...
3. 保存 v2 JSON 报告。
4. 对该报告立即执行 v2 绘图，只生成 PNG、HR CSV 和 error CSV。
5. 写入批量汇总 CSV。
...
- `search_space`
- `qc`
- `figure_paths` 或批量汇总中的 PNG/CSV 输出路径

## 输出目录与命名规范

如果用户未显式选择输出目录，v2 批量全流程默认在输入数据文件所在的统一目录下创建一次运行目录：

- `input_dir/v2_batch_outputs/YYYYMMDD_HHMMSS/`

如果用户选择了输出目录，则直接使用该目录作为本次运行根目录。运行根目录内部固定按文件格式分类：

- `json/`：保存 v2 JSON 报告。
- `png/`：保存 v2 心率结果 PNG 图；本次不生成 PDF 和 SVG。
- `csv/`：保存 HR CSV、error CSV 和批量汇总 CSV。

文件命名使用同一个规范化前缀，保证三类文件可以一眼对应同一次配置：

- 前缀格式：`{sample_stem}-{ppg_mode}-{adaptive_filter}-{analysis_scope}-{reference_order_key}`
- 示例：`tiaosheng2-green-lms-full-HF`
- JSON：`json/{prefix}-v2.json`
- PNG：`png/{prefix}-v2-hr.png`
- HR CSV：`csv/{prefix}-v2-hr.csv`
- error CSV：`csv/{prefix}-v2-error.csv`
- 批量汇总：`csv/v2_batch_summary.csv`

命名中的 `reference_order_key` 使用 `HF`、`HF+CF`、`ACC+HF` 或 `FFT` 这类稳定键。文件名禁止包含空格、中文标点或路径分隔符；后续如出现特殊样本名，需要先做安全化处理。
...
3. 对每个通道运行 v2 Bayes。
4. 保存报告后调用 `render_v2_report(report_path, out_dir=png_dir, csv_dir=csv_dir)` 或等价接口。
5. 记录报告和绘图 artefact。
6. 写 `csv/v2_batch_summary.csv`。
...
4. v2 批量全流程输出 JSON 后自动生成 PNG 和 CSV，不生成 PDF/SVG。
5. 默认输出根目录位于输入数据目录下的 `v2_batch_outputs/YYYYMMDD_HHMMSS/`，并按 `json/`、`png/`、`csv/` 三类子文件夹存放。
6. 批量汇总 CSV 记录报告、PNG、HR CSV、error CSV 和最优误差。
7. v1 旧批量流程和结果分析测试不被破坏。
8. `data/trytry/tiaosheng2` 固定配置验证能说明 v1 `Fusion(HF)` 与 v2 `HF` 是否一致；若不一致，输出足够定位的信息，不允许静默通过。
