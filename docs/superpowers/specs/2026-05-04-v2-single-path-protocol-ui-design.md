...
4. 不在本次设计中重做全新的绘图体系；v2 批量绘图沿用项目已有科研绘图风格，并针对单路径协议更新颜色、图例和误差表语义。
...
- 沿用项目“科研绘图”级风格，优先复用 `skills/publication-plotting` 的规则、`plot_style.py`、`assets/*.mplstyle` 和现有结果分析图的版式习惯。
- 默认输出至少包含 600 dpi PNG；最终论文用途应支持 PDF 和 SVG。
- 心率时间序列图不使用默认 Matplotlib 配色，不使用高饱和纯色。
- 参考 HR 使用深灰或黑灰实线；纯 FFT 基线使用中性灰色虚线；v2 单路径 adaptive 结果使用与参考信号组合绑定的固定颜色。
- 运动段、恢复期等背景标注使用低饱和灰蓝或浅灰透明色，不能压过曲线。
- 图例、误差表和曲线标签必须同步使用 v2 单路径语义，不再出现 v1 的 `HF LMS` / `ACC LMS` 双路径标签。

参考信号组合配色：

- 颜色键使用有序参考组组合，而不是只看集合。`HF+ACC` 与 `ACC+HF` 必须视为不同组合，并使用不同颜色。
- 常见组合需要固定颜色，保证不同批次、不同图之间含义一致：
  - `HF`
  - `CF`
  - `ACC`
  - `HF+CF`
  ...
  - `HF+ACC+CF`
  - 其它顺序组合按同一调色板稳定分配。
- 调色板偏好为“明亮但低饱和”，并满足色盲友好和灰度打印可区分；线型、线宽和稀疏 marker 作为颜色之外的辅助区分手段。

误差表和图例：

- 误差表列名从 v1 的 HF/ACC 双路径更新为 v2 的 `FFT`、`Adaptive(<组合>)`、`Fused/Final` 或更短的等价标签。
- 图例显示参考信号组合，例如 `Adaptive HF+CF`、`Adaptive ACC+HF`。
- 参数表和图内摘要必须记录 `reference_groups_order`，避免相同算法但不同串级顺序被误读为同一方案。
- 批量汇总表增加参考组合字段，支持按 `adaptive_filter + reference_groups_order + ppg_mode + analysis_scope` 分组比较误差。
...
- v2 参数缺失时默认值生效。
- v2 参考信号组合颜色映射稳定，且 `HF+ACC` 与 `ACC+HF` 返回不同颜色。
...
- v2 批量绘图的图例、误差表和汇总表使用单路径标签，不再出现 v1 的 HF/ACC 双路径标签。
...
- v2 批量绘图页面包含刷新按钮。
- v2 批量绘图页面能展示参考组合字段，并在生成结果中保留组合标签。
...
6. v2 批量绘图沿用科研绘图风格，参考信号组合颜色稳定，误差表和图例采用单路径语义。
7. 相关测试通过，或明确记录无法运行测试的环境原因。
