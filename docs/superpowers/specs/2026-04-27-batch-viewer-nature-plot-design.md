...
5. 最终只导出高分辨率 PNG，`dpi >= 600`；默认保存到对应 `.json` 报告所在目录，用户手动选择输出目录时按所选目录保存。
...
7. 误差矩阵作为小型文本注释放在静息段曲线上方，内容包含三种方案在全段和运动段的 MAE：
   - `FFT`
   - `HF-LMS`
   - `ACC-LMS`
...
2. 生成验证：
   ...
   - 确认 PNG 成功生成到指定示例输出目录；默认 GUI/批量行为仍以 `.json` 报告所在目录为输出位置。
...
4. 生成图为 Nature 单栏尺寸、600 dpi PNG，默认位于 `.json` 报告所在目录；手动指定输出目录时位于指定目录。
5. 图中 HF-LMS 是视觉重点，整体配色明亮、柔和、低饱和。
6. 图例仅包含 `Reference`、`FFT`、`HF-LMS`、`ACC-LMS`。
7. 图面无长标题，保留全段和运动段 MAE 简表。
