...
以下规则已由项目负责人明确批准，进入测试优先实现与全新正式重跑。
...
测试采用失败先行方式证明旧实现的 `fill` 恢复历史与不中断历史不同；修复后，恢复历史完全一致。受影响测试集结果为 `104 passed`，Ruff 检查通过。正式实验仍须绑定包含本节修复的最终新提交，并在新的结果根目录从第 1 条记录重跑。
