...
切换层 `NO-GO`。三种执行方式消费完全相同的冻结 N2 handoff 与 `switch_target_ready`。hard gap rescue 在大漂移样本上最快且优于 bounded，但 bobi2 和 kaihe2 的固定 60 s Final MAE 仍分别为 10.45 和 15.21 BPM，均未达到不高于 3 BPM且 E20=0 的绝对门槛；因此不能冻结为主算法切换方案。

## 机制修正

旧 gap rescue 只识别 `Final - reset >= gap`，会遗漏 kaihe2 这类 Final 低锁、可信 handoff 较高的反向大差。新实验路径在 `switch_target_ready` 已成立时使用绝对 gap；旧生产默认仍保持单向行为，避免在方案未通过前改变现有算法。gap 和目标稳定性累计也只从 ready 建立后开始，不能借用未就绪窗口直接硬切。
...
|---|---|---:|---:|---:|---:|
| bobi2 | hard | gap / 21 s | 10.454 | 20 | 21.572 |
| bobi2 | bounded | gap / 21 s | 12.301 | 21 | 1.500 |
| bobi2 | stable | crossover / 57 s | 18.207 | 24 | 0.165 |
| kaihe2 | hard | gap / 17 s | 15.210 | 10 | 84.961 |
| kaihe2 | bounded | gap / 17 s | 36.222 | 36 | 2.021 |
...
D2 五条样本没有新增 E20，hard 相对旧 Final 的最大 MAE 退化约 0.12 BPM，说明 ready 门控的安全性有效；这不能替代 D1 绝对门槛。
