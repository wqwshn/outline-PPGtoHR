...
本轮定位并修复了 `gap_rescue` 成功硬切后因可观测性短暂下降而跳回高漂移 archived Final 的机制错误。修复对 `xiezi2` 有效，并将 `run2` 的实验 E20 从修复前的 23 降至 9；但 HB24 仍有 `run1/run2/xiezi3` 三个正常样本未通过逐样本防退化门槛，因此整体结论为 **NO-GO**，实验机制继续保持默认关闭。
...
该修复没有放宽硬切资格，也没有让独立 reset FFT 参与 Final。
...
- `kaihe3`：21.899 → 14.712 BPM，仍未达到绝对门槛。
