...
提案包含 36 个新诊断身份，最坏尝试预算为 72。用户随后批准了该精确提案与
v8 预算，36/36 个身份已成功执行，参数搜索与独立 BO 运行数仍为 0。修正后的
结果没有产生 12/12 完整通过档位：`p25-short-low` 为 10/12，
`p25-short-mid` 为 5/12，`p25-long-mid` 为 3/12。完整证据和下一人工门见
[p25 修正量纲频谱复验报告](2026-07-30-lyx-p25-spectral-recheck-result.md)。
...
| p25 档位在 v2 下安全 | 已完成 3×12 复验；最佳档位仅 10/12 完整通过 | 不支持 12/12 安全提名 |
