...
- 普通 Lite 默认配置未改变。
- 审计 HB Lite 批量命令现在必须显式提供固定参数判定文件；判定不是 `GO`、未明确 `bo_allowed=true` 或未记录冻结候选时，会在创建批量输出前失败。
...
进入 #57/#58 前仍有三项明确风险：当前候选丢失 `bobi2` 的既有 `<3 BPM` 救援；正常样本总体或单样本可能退化；当前审计批量入口仍对应旧 N5 双 reset 配置，未来必须在固定判定中同时绑定实际精简候选配置，不能只凭候选名称启动。最终停止结论应回链父 Issue #59，但本任务不自动关闭父 Issue。
